...
int criaMatriz(Numero **matriz, int *TAM, int *estadoPL, int *estadoPD); // Função responsável por criar uma matriz.
...
int verificaVitoria(JogoSumplete jogo); // Verifica se as condições de vitória estão satisfeitas.
